...
public class CrystalChams implements ModInitializer {
    ...
    public static EvilOption<Easings> createEasingOption(StateManager<Easings> stateManager, OptionGroups group) {
        return EvilOption.<Easings>createBuilder().name(Text.of(SEPARATOR + "config.option.easing")).stateManager(stateManager).description(getDescription("yeah")).controller(easingsOption -> EnumControllerBuilder.create(easingsOption).enumClass(Easings.class)).group(group).build();
    }
    ...
    public static EvilOption<Integer> createSkyLightLevelOption(StateManager<Integer> stateManager) {
        return EvilOption.<Integer>createBuilder()
                ...
                .controller(integerOption -> CustomIntegerSliderControllerBuilder.create(integerOption).step(1).range(-1, 15).formatValue(CrystalChams.LIGHT_FORMATTER))
...
//                    ((Option<RenderMode>) value).requestSet(RenderMode.values()[safeRandom(0, RenderMode.values().length - 1)]);
    ...
    public static void renderCustomBeam(float dx, float dy, float dz, float tickDelta, int age, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int mode) {
        /*
        //TODO
        0 - non-preview
        1 - preview
        ...
        for (int n = 1; n <= ChamsConfig.o_beamSides.pendingValue(); n++) {
            ...
            int endCol = ChamsConfig.o_beam2Rainbow.pendingValue() ? ColorHelper.Argb.getArgb((int) (ChamsConfig.o_beam2Alpha.pendingValue() * (mode == 1 ? CrystalChams.beamProgress : 1) * 255F), rainbowCol2.getRed(), rainbowCol2.getGreen(), rainbowCol2.getBlue()) : ColorHelper.Argb.getArgb((int) (ChamsConfig.o_beam2Alpha.pendingValue() * (mode == 1 ? CrystalChams.beamProgress : 1) * 255F), ChamsConfig.o_beam2Color.pendingValue().getRed(), ChamsConfig.o_beam2Color.pendingValue().getGreen(), ChamsConfig.o_beam2Color.pendingValue().getBlue());
            vertexConsumer.vertex(entry, k * ChamsConfig.o_beam2Radius.pendingValue(), l * ChamsConfig.o_beam2Radius.pendingValue(), 0.0F).color(endCol).texture(m, h).overlay(OverlayTexture.DEFAULT_UV).light(packLight(ChamsConfig.o_beam2BlockLightLevel.pendingValue(), ChamsConfig.o_beam2SkyLightLevel.pendingValue(), light)).normal(entry, 0.0F, -1.0F, 0.0F);
            vertexConsumer.vertex(entry, k * ChamsConfig.o_beam1Radius.pendingValue(), l * ChamsConfig.o_beam1Radius.pendingValue(), g).color(startCol).texture(m, i).overlay(OverlayTexture.DEFAULT_UV).light(packLight(ChamsConfig.o_beam1BlockLightLevel.pendingValue(), ChamsConfig.o_beam1SkyLightLevel.pendingValue(), light)).normal(entry, 0.0F, -1.0F, 0.0F);
            vertexConsumer.vertex(entry, o * ChamsConfig.o_beam1Radius.pendingValue(), p * ChamsConfig.o_beam1Radius.pendingValue(), g).color(startCol).texture(q, i).overlay(OverlayTexture.DEFAULT_UV).light(packLight(ChamsConfig.o_beam1BlockLightLevel.pendingValue(), ChamsConfig.o_beam1SkyLightLevel.pendingValue(), light)).normal(entry, 0.0F, -1.0F, 0.0F);
