...
public class CustomFloatSliderControllerBuilderImpl extends AbstractControllerBuilderImpl<Float> implements FloatSliderControllerBuilder {
    private float min, max;
    private float step;
    private ValueFormatter<Float> formatter = FloatSliderController.DEFAULT_FORMATTER::apply;

    public CustomFloatSliderControllerBuilderImpl(Option<Float> option) {
        super(option);
    }
    ...
    public Controller<Float> build() {
        return CustomFloatSliderController.createInternal(option, min, max, step, formatter);
    }
}
